...
class ClientSummary(BaseModel):
    ...
    # ── SentinelOne Rich Telemetry ──
    s1_threat_classifications: list[dict] = Field(default_factory=list)   # [{label, count}]
    s1_threat_mitigations:     list[dict] = Field(default_factory=list)   # [{label, count}]
    s1_threat_files:           list[dict] = Field(default_factory=list)   # [{label, count}] top threat file names
    s1_recent_threats:         list[dict] = Field(default_factory=list)   # detailed threat rows
    s1_vuln_apps:              list[dict] = Field(default_factory=list)   # [{label, count}] app+version
    s1_vuln_severity:          list[dict] = Field(default_factory=list)   # [{label, count}] severity
    s1_vuln_endpoints:         list[dict] = Field(default_factory=list)   # [{label, count}] top vuln endpoints
    s1_os_distribution:        list[dict] = Field(default_factory=list)   # [{label, count}] OS types
    s1_agent_versions:         list[dict] = Field(default_factory=list)   # [{label, count}] version coverage
    s1_agents_attention:       list[dict] = Field(default_factory=list)   # [{label, count}] attention categories
    s1_endpoint_names:         list[str]  = Field(default_factory=list)   # all computer names
