"""
PDF → XLSX Converter — Backend Logic
Ported from conversion.py (Streamlit) to plain Python for Flask.
"""
import io, re
...
def _is(text):
    return bool(re.match(r'(Top\s+Threats?\s+By\s+|Applications?\s+Summary|ZoneWAF)',
                         text, re.IGNORECASE))
...
def _extract_widgets_from_text(page_text):
    ...
    list_title_re = re.compile(
        r'Top\s+\d+\s+(Countries|Sites|Attack\s+Categories|Targeted\s+Applications|IPs).*',
        re.IGNORECASE)
    ranked_re = re.compile(r'^(\d+)\s+(.+?)\s+([\d.,]+\s*[KkMmBbTt]?)$')
    pair_re   = re.compile(r'^(.+?)\s{2,}([\d.,]+\s*[KkMmBbTt]?)$')
    cur_list  = None; cur_rank = 0
    for i, ln in enumerate(raw_lines):
        ...
        if cur_list is not None:
            ...
            if rm:
                lists[cur_list].append([rm.group(1), rm.group(2).strip(), rm.group(3).strip()]); continue
            pm = pair_re.match(ln)
            if pm:
                name_part = pm.group(1).strip(); val_part = pm.group(2).strip()
                if not list_title_re.search(name_part) and not re.match(
                        r'^(Threat|Security|Bot|DDoS|Network|Platform|Compliance)', name_part, re.IGNORECASE):
                    cur_rank += 1
                    lists[cur_list].append([str(cur_rank), name_part, val_part]); continue
            if list_title_re.search(ln) or re.match(
                    r'^(Threat|Security|Bot|DDoS|Network|Platform|Compliance|Compared)', ln, re.IGNORECASE):
                cur_list = None
    inline_re = re.compile(
        r'^(Total\s+Attacks\s+Blocked|Attacks\s+Blocked\s+by\s+Custom\s+Rules|'
        r'Total\s+Vulnerabilities\s+Detected|WAF\s+Rules\s+Created|Custom\s+Rules\s+Created|'
        r'Total\s+Bandwidth\s+Used|SwyftComply\s+Requested|Scans\s+Completed|'
        r'Manual\s+Pentesting\s+Requested|POC\s+Requested|Total\s+DDoS\s+Attacks\s+Blocked|'
        r'Sites\s+under\s+DDoS\s+Attack|Sites\s+under\s+Bot\s+Attack|Bot\s+Attacks\s+Blocked)'
        r'\s+([\d.,]+\s*[KkMmBbTtGgBb]*)\s*(.*)', re.IGNORECASE)
    block_labels = ["Sites under Bot Attack","Bot Attacks Blocked","Sites under DDoS Attack","Total DDoS Attacks Blocked"]
    for i, ln in enumerate(raw_lines):
        ll = ln.lower()
        im = inline_re.match(ln)
        if im: metrics[im.group(1).strip()] = [im.group(2).strip(), im.group(3).strip()]; continue
            ...
            if label.lower() in ll and len(ln) < len(label) + 15:
                for j in range(i+1, min(i+4, len(raw_lines))):
                    ...
                    if re.match(r'^[\d.,]+\s*[KkMmBbTt]?$', nt):
                        if label not in metrics: metrics[label] = [nt, ""]
                        break
                break
        if "no targeted applications" in ll: metrics["Targeted Applications"] = ["0","None detected"]
        if "no bot categories detected" in ll: metrics["Bot Categories"] = ["0","None detected"]
...
def convert_pdf_to_xlsx(pdf_bytes: bytes) -> BytesIO:
    ...
    with pdfplumber.open(BytesIO(pdf_bytes)) as pdf:
        for pn, page in enumerate(pdf.pages, 1):
            ...
            if full_text:
                ...
                for k, v in m.items(): all_metrics[k] = v
            w = page.width; h = page.height
            for crop_box in [(0,0,w*0.52,h), (w*0.48,0,w,h)]:
                try:
                    ct = page.crop(crop_box).extract_text(x_tolerance=3, y_tolerance=3)
                    if ct:
            ...
            for tbl in tables:
                ...
                keep = [(i,h) for i,h in enumerate(hdrs) if h["name"].strip().lower() not in EXCLUDED_COLUMNS]
    ...
    if all_metrics:
        ...
        for ri, (k, (v_num, v_det)) in enumerate(all_metrics.items(), 2):
            ...
            val = _efn(v_num); c2 = ws.cell(row=ri, column=2, value=val); _sc(c2, num=_in(val))
    ...
    for list_name, rows in all_lists.items():
        ...
        for ri, r in enumerate(rows, 2):
            ...
            val = _efn(r[2]); c3 = ws.cell(row=ri, column=3, value=val); _sc(c3, num=_in(val))
        _aw(ws)
